...
def valida_cpf(num_cpf):
    ...
    for digits in cpf:        
        if cpf.count(0) > 10: return False
        if cpf.count(1) > 10: return False
        if cpf.count(2) > 10: return False
        if cpf.count(3) > 10: return False
        if cpf.count(4) > 10: return False
        if cpf.count(5) > 10: return False
        if cpf.count(6) > 10: return False
        if cpf.count(7) > 10: return False
        if cpf.count(8) > 10: return False
        if cpf.count(9) > 10: return False
